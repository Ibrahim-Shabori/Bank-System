...
public:
    ...
    enum enPermissions {
        ...
        pLoginRegistersList = 128, pCurrencyExchange = 256
    };
    ...
    {
        _Password = clsUtil::EncryptText(Password, 1);
    }
    ...
    static clsUser Find(string UserName, string Password)
    {
        ...
        if (MyFile.is_open())
        {
            ...
            {
                clsUser User = _ConvertLinetoUserObject(Line);
                if (User.GetUserName() == UserName && clsUtil::DecryptText(User.GetPassword(), 1) == Password)
                {
                    MyFile.close();
                    return User;
                }
    ...
    bool CheckAccessPermission(enPermissions Permissions){
        ...
        if ((this->GetPermissions() & Permissions) == Permissions) return true;
        return false;
    }
